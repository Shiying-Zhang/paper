...
- **隐藏层** $(c_W=1)$：学习率 $\propto 1/n$。隐藏层的输出是 $n$ 个神经元的加权和，每个参数的更新对输出的贡献 $\propto 1$，所以 $n$ 个参数的**总更新** $\propto n \cdot \eta$。要保持总更新 $O(1)$，需要 $\eta \propto 1/n$。
- **输出层** $(a_W=1, c_W=0)$：权重本身 $\propto 1/n$（$a_W=1$），所以即使学习率不缩放，每个参数更新对输出的贡献也是 $\propto 1/n$，总更新已经是 $O(1)$。
- **输入层** $(c_W=1)$：与隐藏层类似的推理。

**核心保证**：在 μP 下，每一层的**特征更新量**在训练过程中保持**宽度无关**——不论模型多宽，每一步的特征变化量相同。这使得最优超参数不随宽度变化。
